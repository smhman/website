...
$.getJSON(`${url}weather?q=${cityName}&appid=${key}&units=metric&lang=en`, function(data) {
    $("#weather").html(`<p class="weatherText"> <span class="text-capitalize">${data.weather[0].description}</span> and ${Math.round(data.main.temp)}°C in <span class="coloredText">${data.name}, ${data.sys.country}</span>`);
});